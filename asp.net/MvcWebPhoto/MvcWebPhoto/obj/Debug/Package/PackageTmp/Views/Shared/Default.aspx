...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <!--Passit BUTTON BEGIN-->
    <table style="margin: 20px 0 0 10px; width: 98%;" cellpadding="6">
        <tr>
            <td align="left">
                <%if (Convert.ToInt32(ViewData["TotalPage"]) > 1 && Convert.ToInt32(ViewData["CurrentPage"]) > 1)
                  { %>
                <a style="font-weight: normal; font-size: 12px;" href="/Home/Default/<%=ViewData["id"] %>/page<%=Convert.ToInt32(ViewData["CurrentPage"])-1 %>">
                    «&nbsp;Older&nbsp;posts&nbsp;上一页</a>
                <%} %>
            </td>
            <td align="right">
                <%if (Convert.ToInt32(ViewData["TotalPage"]) > 1 && Convert.ToInt32(ViewData["CurrentPage"]) < Convert.ToInt32(ViewData["TotalPage"]))
                  { %>
                <a style="font-weight: normal; font-size: 12px;" href="/Home/Default/<%=ViewData["id"] %>/page<%=Convert.ToInt32(ViewData["CurrentPage"])+1 %>">
                    Newer&nbsp;posts&nbsp;下一页&nbsp;»</a>
                <%} %>
